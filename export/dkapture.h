...
class DKapture
{
  public:
	 ...
	 *        对数据进行更新。
	 * @param ms 目标有效时间（单位：毫秒）。如果为
	 * UINT64_MAX，则不修改，仅返回当前设定值
	 * @return 返回旧的设定值
	 */
	...
	/**
	 * @brief
	 * 读取指定进程的多个类型（节点）数据，一次性多个的效率要比多次读取高。
	 * @param dts 数据类型列表
	 * @param pid 目标进程 ID，设置为0时表示读取所有进程(强烈建议不要
	 *          使用该接口读取所有进程信息，因为进程的数量是不确定的，因此接口需要
	 *          buffer也会变化，建议使用支持回调的重载读取所有进程信息)。
	 * @param buf 数据存储缓冲区，多个数据存储在buf中，不保证先后顺序与 dts
	 * 一致。 用户可以通过 DataHdr 中 dsz 字段依次计算偏移遍历所有数据。
	 * @param bsz 缓冲区大小。
	 * @return 成功返回读取的字节数，失败返回 -errno
	 */
	virtual ssize_t
	read(std::vector<DataType> &dts, pid_t pid, DataHdr *buf, size_t bsz) = 0;

	/**
	 * @brief
	 * 从多个路径中读取数据，和上一重载类似，但更灵活，可以通过路径交叉指定
	 *      多个进程和多个数据类型，例如 </proc/1/stat, /proc/2/statm,
	 * /proc/2/io>
	 * @param paths 数据路径列表
	 * @param buf 数据存储缓冲区，多个数据存储在buf中，不保证先后顺序与 paths
	 * 一致。 用户可以通过 DataHdr 中 dsz 字段依次计算偏移遍历所有数据。
	 * @param bsz 缓冲区大小
	 * @return 成功返回读取的字节数，失败返回 -errno
	 */
	virtual ssize_t
	read(std::vector<const char *> &paths, DataHdr *buf, size_t bsz) = 0;
	 ...
	 */
	virtual ssize_t
	read(DataType dt, std::vector<pid_t> &pids, DataHdr *buf, size_t bsz) = 0;
	 ...
	 */
	virtual ssize_t
	read(std::vector<DataType> &dts, DKCallback cb, void *ctx) = 0;
	 ...
	 */
	virtual ssize_t
	read(std::vector<const char *> &paths, DKCallback cb, void *ctx) = 0;
...
struct XattrLog : public FileLog
{
	...
	size_t size;
	long ret;
	char action[]; // must be less than 4096 - sizeof(FileLog) -
				   // sizeof(XattrLog)
} __attribute__((__packed__));
